...
T2_list = [1300, 1400, 1500, 1600, 1700, 1800, 
           ...
           2500, 2600, 2700, 2800]
Tphi_list_10 = [(1/T2_list[indx] - 0.5/T1_list[0])**(-1) for indx in range(len(T2_list))]
Tphi_list_15 = [(1/T2_list[indx] - 0.5/T1_list[1])**(-1) for indx in range(len(T2_list))]
...
axs[0].scatter(Tphi_list_10, fid_matrix[0, :], label = f"T1 = {T1_list[0]/1000}us")
axs[0].scatter(Tphi_list_15, fid_matrix[1, :], label = f"T1 = {T1_list[1]/1000}us")
axs[0].plot([min(Tphi_list_10), max(T2_list)], [fid_T1_A, fid_T1_A], linestyle = "--", label = "T1 = 10 us, Tphi = inf")
axs[0].plot([min(Tphi_list_15), max(T2_list)], [fid_T1_B, fid_T1_B], linestyle = "--", label = "T1 = 15 us, Tphi = inf")
# axs[0].plot([min(Tphi_list), max(T2_list)], [fid_leak, fid_leak], linestyle = "--", color = "k", label = "T1 = Tphi = inf")

axs[0].set_title("a0 + b2 + b4")
...
axs[1].scatter(Tphi_list_10, fid_matrix[0, :], label = f"T1 = {T1_list[0]/1000}us")
axs[1].scatter(Tphi_list_15, fid_matrix[1, :], label = f"T1 = {T1_list[1]/1000}us")
axs[1].plot([1500, 3000], [fid_T1_A, fid_T1_A], linestyle = "--", label = "T1 = 10 us, Tphi = inf")
axs[1].plot([1500, 3000], [fid_T1_B, fid_T1_B], linestyle = "--", label = "T1 = 15 us, Tphi = inf")
axs[1].plot([1500, 3000], [fid_leak, fid_leak], linestyle = "--", color = "k", label = "T1 = Tphi = inf")
...
plt.savefig("sim_stateprep_fids.pdf")
plt.show()