...
T1_list = [10000, 15000]
T2_list = np.arange(1300, 2801, 100)
# T1_list = [np.inf]
# T2_list = [np.inf]
Tphi_list_10k = 1/(1/np.array(T2_list) - 1/2/10000)
Tphi_list_15k = 1/(1/np.array(T2_list) - 1/2/15000)
...
for T1 in T1_list:
    for T2 in T2_list:
        ...
        state_fname = f"rho_0p3_dims6x2_T1{T1}_T2{T2}_cavT1500000.npz"
        ...
        fid_list.append(B_fid_mean)
        # print(f'Fidelity is {Fid}')
        # np.savez("reconstructed_0p3_T22700_T115000_cavT1_500000", rho = B_qRho_est)


fid_matrix = np.array(fid_list).reshape(2,-1)
plt.scatter(Tphi_list_10k, fid_matrix[0,:], label = f"T1 = 10us")
plt.scatter(Tphi_list_15k, fid_matrix[1,:], label = f"T1 = 15us")
plt.legend()
plt.show()