...
for T1 in T1_list:
    for T2 in T2_list:
        ...
        state_fname = f"rho_0p2p4_dims6x2_T1{T1}_T2{T2}_cavT1500000.npz"
        ...
        # np.savez(f"reconstructed_0p2p4_T2{T2}_T1{T1}_cavT1_500000", rho = B_qRho_est)


fid_matrix = np.array(fid_list).reshape(2,-1)
print(fid_matrix)
for i in range(len(T1_list)):
    plt.plot(T2_list, fid_matrix[i,:], label = f"T1 = {T1_list[i]}")
plt.legend()
plt.show()