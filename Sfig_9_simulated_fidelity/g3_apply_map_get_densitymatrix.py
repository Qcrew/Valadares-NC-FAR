...
T2_list = [1500, 2000, 2500, 3000, 3500, 4000, 4500]
fid_list = []
for i in range(len(theta_list)):
    theta = theta_list[i]
    theta_label = theta_label_list[i]
    for T2 in T2_list:
        # # Experimental observables
        state_fname = f"rho_1to4_theta" + theta_label + f"_dims6x2_T110000_T2{T2}_cavT1500000.npz"
        ...
        target_ket = ((np.cos(theta/2))**2*basis(D,1) + (np.sin(theta/2))**2*basis(D,4)).unit()
...
fid_matrix = np.array(fid_list).reshape(len(theta_label_list),-1)
print(fid_matrix)
for i, state_label in enumerate(theta_label_list):
    plt.plot(T2_list, fid_matrix[i,:], label = state_label)
plt.legend()
plt.show()